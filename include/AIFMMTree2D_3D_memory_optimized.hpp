...
// public:
	FMM2DTree(inputsToSolverClass inputs, kerneltype* K) {
		...
		nChild = std::pow(2,nDims);// for 3d ni. of children is 8
		...
		#ifdef USE_TWO
			// this->nLevels = log(N/inputs.leafSize)/log(4);
			this->nLevels = std::max(2, int(std::floor(log(N/inputs.leafSize)/log(4))));
			eta = std::sqrt(2)*2;
		#elif USE_THREE
			// this->nLevels =	log(N/inputs.leafSize)/log(8);
			this->nLevels =	std::max(2, int(std::floor(log(N/inputs.leafSize)/log(8))));
			eta = std::sqrt(3)*2;
		#endif
		...
		// fillin_rankVec.push_back(0);

		int temp = 5;
		int i = 0;
		for (int k=nLevels; k>=2; --k) {
			fillin_rankVec[k] = temp;
			++i;
			if (i%3 == 0) {
				temp += 5;
			}
		...
		// for (int k=nLevels; k>=2; --k) {
		// 	fillin_rankVec[k] = fillin_rank;
		// }

		for (int k=0; k<=nLevels; ++k) {
			std::cout << "j: " << k << " fillin_rankVec: " << fillin_rankVec[k] << std::endl;
		}
	}
	...
	double distanceBetweenCenters(int jk, int k, int jb, int b) {
		...
		double distCenters = distCentersX*distCentersX + distCentersY*distCentersY;
		#elif USE_THREE
		double distCentersX = tree->at(jk)[k].center.x - tree->at(jb)[b].center.x;
		double distCentersY = tree->at(jk)[k].center.y - tree->at(jb)[b].center.y;
	...
	bool admissibilityCondition(int j, int child, int other) {
		...
		if (distCenters > etaLocal*boxSideLength[j] + boxSideLength[nLevels]/2.0) {
	...
	// }
		...
		void assign_Center_Location() {
			...
			for (int j=0; j<nLevels; ++j) {
				...
				// #pragma omp parallel for
				for (int k=0; k<nBoxesPerLevel[j]; ++k) {
					tree->at(J)[4*k].center.x		=	tree->at(j)[k].center.x-shift;
					tree->at(J)[4*k+1].center.x	=	tree->at(j)[k].center.x+shift;
					tree->at(J)[4*k+2].center.x	=	tree->at(j)[k].center.x+shift;
					tree->at(J)[4*k+3].center.x	=	tree->at(j)[k].center.x-shift;

					tree->at(J)[4*k].center.y		=	tree->at(j)[k].center.y-shift;
					tree->at(J)[4*k+1].center.y	=	tree->at(j)[k].center.y-shift;
					tree->at(J)[4*k+2].center.y	=	tree->at(j)[k].center.y+shift;
					tree->at(J)[4*k+3].center.y	=	tree->at(j)[k].center.y+shift;
				}
			}
		}
	#elif USE_THREE
...
void reorder(Vec &potential) {
	...
	int getAvgMultipoles() {
		...
		for (int j = nLevels; j >= startLevel; j--) {
			...
			int nBox_level = 0;
			for (size_t k = 0; k < nBoxesPerLevel[j]; k++) {
				if (tree->at(j)[k].P2M.rows() > 0 && tree->at(j)[k].interactionList.size() > 0) {
					nBox_level++;
					avg_level += tree->at(j)[k].P2M.rows();
					// std::cout << "k: " << k << "	tree->at(j)[k].P2M.rows(): " << tree->at(j)[k].P2M.rows() << std::endl;
					// nBox++;
					// avg += tree->at(j)[k].P2M.rows();
				}
			}
			std::cout << "j: " << j << "	avg_rank: " << avg_level/nBox_level << std::endl;
			avg += avg_level;
			nBox += nBox_level;
		}
		std::cout << "overall avg_rank: " << avg/nBox << std::endl;
		return avg/nBox;
	}
	...
	void get_L2P_P2M_box(int j, int k) {
		...
		// delete &tree->at(j)[k].Ar;
		// delete &tree->at(j)[k].Ac;
		// tree->at(j)[k].Ar = Mat(0,0);
		// tree->at(j)[k].Ac = Mat(0,0);
		// Eigen::internal::aligned_free(tree->at(j)[k].Ar);
		// Eigen::internal::aligned_free(tree->at(j)[k].Ac);
		// tree->at(j)[k].Ar.destroy();
		// tree->at(j)[k].Ac.destroy();
	}
...
void write_NumMultipoles() {
    ...
    for (int k=0; k<nBoxesPerLevel[j]; ++k) {
			...
			// std::cout << "Time to getNodes_incoming_box(j, k): " << end-start << std::endl;
			// std::cout << "Avg Time to 1 getMatrixEntryFromScratch: " << K->getMatrixEntryFromScratchTime/K->ngetMatrixEntryFromScratch << std::endl;
			// std::cout << "Avg Time to 1 getMatrixEntry: " << K->getMatrixEntryTime/K->ngetMatrixEntry << std::endl;
    ...
    std::vector<int> boxA_Nodes;
		...
		if (n_cols > 0 && n_rows > 0) {
			// Mat mat = K->getMatrix(IL_Nodes, boxA_Nodes);
			// // Eigen::JacobiSVD<Eigen::MatrixXcd, Eigen::ComputeThinU | Eigen::ComputeThinV> svd(mat);
			...
			// 	if (sv(i) < pow(10,-TOL_POW)) {
			// 		svdRank = i;
			// 		break;
			// 	}
			// }
			// Eigen::VectorXd svthin = svd.singularValues().segment(0,svdRank);
			// Eigen::MatrixXcd Uthin = U.block(0,0,n_rows,svdRank);
			// Eigen::MatrixXcd Vthin = V.block(0,0,n_cols,svdRank);
			//
			// Eigen::MatrixXd singularValueMatrix = svthin.asDiagonal();
			// Eigen::MatrixXcd errMatrix = mat - Uthin*singularValueMatrix*Vthin.adjoint();
			// double errSVD = errMatrix.norm()/mat.norm();
			// // std::cout << "err in SVD: " << errSVD << std::endl;
			// std::cout << "O j: " << j << "	k: " << k << "	n_rows: " << n_rows << "	n_cols: " << n_cols << "	rank: " << svdRank << "	err: " << errSVD << std::endl;
			...
			if(ComputedRank > 0) {
				...
				// std::cout << "O j: " << j << "	k: " << k << "	n_rows: " << n_rows << "	n_cols: " << n_cols << "	rank: " << ComputedRank << "	err: " << err << std::endl;
 				// std::cout << std::endl;
				// std::cout << "O j: " << j << "	k: " << k << "	n_rows: " << n_rows << "	n_cols: " << n_cols << "	rank: " << ComputedRank << std::endl;
			...
			// 	tree->at(j)[k].P2M = Mat(tree->at(j)[k].outgoing_checkPoints.size(),tree->at(j)[k].Ar.cols());
			// }
			// std::cout << "O j: " << j << "	k: "<< k << "	ComputedRank: " << ComputedRank << std::endl;
		}
		else if (n_rows == 0) {
  ...
  // void getNodes_incoming_box(int j, int k, int& n_rows, int& n_cols, int& ComputedRank) {
    ...
    std::vector<int> boxA_Nodes;
		...
		// std::cout << "I: n_rows: " << n_rows << "	n_cols: " << n_cols << std::endl;
		if (n_rows > 0 && n_cols > 0) {
			// Mat mat = K->getMatrix(boxA_Nodes, IL_Nodes);
			// // Eigen::JacobiSVD<Eigen::MatrixXcd, Eigen::ComputeThinU | Eigen::ComputeThinV> svd(mat);
			// Eigen::JacobiSVD<Eigen::MatrixXcd> svd;
			// svd.compute(mat, ComputeThinU | ComputeThinV);
			// Eigen::MatrixXcd U = svd.matrixU();
			...
			if(ComputedRank > 0) {
				...
				// double err = Err.norm()/mat.norm();
				// std::cout << "I j: " << j << "	k: " << k << "	n_rows: " << n_rows << "	n_cols: " << n_cols << "	rank: " << ComputedRank << "	err: " << err << std::endl;
				// std::cout << std::endl;
				// std::cout << "I j: " << j << "	k: " << k << "	n_rows: " << n_rows << "	n_cols: " << n_cols << "	rank: " << ComputedRank << std::endl;
			}
			delete LR;
			// else {
			// 	tree->at(j)[k].L2P = Mat(tree->at(j)[k].Ac.rows(), tree->at(j)[k].incoming_checkPoints.size());
			// }
	    // std::cout << "I j: " << j << "	k: "<< k << "	ComputedRank: " << ComputedRank << std::endl;
		}
		else if (n_cols == 0) {
			tree->at(j)[k].ILActive = false;
	...
	void free_charge_and_checkPoints() {
		for (size_t j = startLevel; j <= nLevels; j++) {
			for (size_t k = 0; k < nBoxesPerLevel[j]; k++) {
				tree->at(j)[k].outgoing_chargePoints.clear();
				tree->at(j)[k].incoming_checkPoints.clear();
				tree->at(j)[k].Ar = Mat(0,0);
				tree->at(j)[k].Ac = Mat(0,0);
			}
		}
	...
	void compress_P2P_myRRQR(int j, int k, int b, Mat old_L2P, Mat old_P2M, Mat P2PToBeCompressed, Mat &new_L2P, Mat &new_P2M_transpose, Mat &Rk, Mat &Rn, Mat &Rn_prime, Mat& newM2L) {
		...
		// if (j==3 && k==231 && b==239) {
			// std::cout << "1" << std::endl;
		// }
		RRQR *rrqr = new RRQR(A_col, TOL_POW, fill_in_TOL_POW, old_L2P.cols(), tree->at(j)[k].maxPivot_L2P,0, fillin_rankVec[j]);
		...
		// std::cout << "P2P err1: " << err1 << std::endl;
		// if (j==3 && k==231 && b==239) {
			// std::cout << "3" << std::endl;
		// }
	  ...
	  // std::cout << "errQ: " << errQ << std::endl;
		...
		// }
		RRQR *rrqr2 = new RRQR(A_row, TOL_POW, fill_in_TOL_POW, old_P2M.rows(), tree->at(j)[b].maxPivot_P2M,0, fillin_rankVec[j]);
		// if (j==3 && k==231 && b==239) {
			// std::cout << "5" << std::endl;
		// }
		rrqr2->colPivHouseholderQRWithPartition();
		double err2 = rrqr2->errorInQR();
		std::cout << "P2P err2: " << err2 << std::endl;
		// if (j==3 && k==231 && b==239) {
			// std::cout << "6" << std::endl;
		// }
		// std::cout << "done" << std::endl;
	...
	void compress_M2P_myRRQR(int j, int k, int b, Mat &new_L2P, Mat &M2L) {
		...
		RRQR *rrqr = new RRQR(A_col, TOL_POW, fill_in_TOL_POW, tree->at(j)[k].L2P.cols(), tree->at(j)[k].maxPivot_L2P,0, fillin_rankVec[j]);
		rrqr->colPivHouseholderQRWithPartition();
		// double err1 = rrqr->errorInQR();
		// std::cout << "M2P err1: " << err1 << std::endl;
	...
	void compress_P2L_myRRQR(int j, int k, int b, Mat &new_P2M_transpose, Mat &M2L) {
		...
		// 	std::cout << "A" << std::endl;
		// }
		RRQR *rrqr = new RRQR(A_row, TOL_POW, fill_in_TOL_POW, tree->at(j)[b].P2M.adjoint().cols(), tree->at(j)[b].maxPivot_P2M,0, fillin_rankVec[j]);
		...
		// }
		rrqr->colPivHouseholderQRWithPartition();
		double err2 = rrqr->errorInQR();
		std::cout << "P2L err2: " << err2 << std::endl;
		// if (j==6 && b==151527 && k==151359) {
		// 	std::cout << "C" << std::endl;
		// }
	...
	void update_parent_L2P(int j, int k, Mat &L) {
		...
		#ifdef USE_TWO
			tree->at(j-1)[kP].L2P << U[0], U[1], U[2], U[3];
		#elif USE_THREE
	...
	void update_parent_P2M(int j, int b, Mat &R) {
		...
		#ifdef USE_TWO
			tree->at(j-1)[bP].P2M << V[0], V[1], V[2], V[3];
		#elif USE_THREE
			...
			for(int n=0; n<tree->at(j)[k].neighborList.size(); ++n) {
				...
				if(nn != k) {
					for(int a=0; a<nChild; ++a) {
						for(int b=0; b<nChild; ++b) {
							if (tree->at(j+1)[nChild*k+a].M2L[nChild*nn+b].size() > 0) {
								tree->at(j+1)[nChild*k+a].M2L[nChild*nn+b] = Mat(0,0);
							}
						}
					}
				}
    ...
    for (size_t k = 0; k < nBoxesPerLevel[j]; k++) {
        ...
        particle_rhs_length += tree->at(j+1)[nChild*k+c].multipole_rhs.size();
			...
			#ifdef USE_TWO
				tree->at(j)[k].particle_rhs << tree->at(j+1)[nChild*k+0].multipole_rhs, tree->at(j+1)[nChild*k+1].multipole_rhs, tree->at(j+1)[nChild*k+2].multipole_rhs, tree->at(j+1)[nChild*k+3].multipole_rhs;
			#elif USE_THREE
...
void rhs_filings_in_equation_P2M_due_to_x(int j, int k, Eigen::PartialPivLU<Mat>& P2P_self_QR) {
		...
		for (size_t n = 0; n < tree->at(j)[k].neighborList.size(); n++) {
			...
			if(nn != k) {
				if (!tree->at(j)[nn].Eliminated) {
					...
					// std::cout << "is_well_separated(j,k,nn): " << is_well_separated(j,k,nn) << std::endl;
					if(is_well_separated(j,k,nn)) {
						// std::cout << "1: " << tree->at(j)[k].M2L[nn].norm() << "	2: " << tree->at(j)[k].P2L[nn].norm()/tree->at(j)[k].M2L[nn].norm() << std::endl;
						if (tree->at(j)[k].M2L[nn].norm() > 0 && tree->at(j)[k].P2L[nn].norm()/tree->at(j)[k].M2L[nn].norm() ) {
  ...
  void rhs_filings_in_equation_P2P_due_to_z(int j, int k, int nn, Eigen::PartialPivLU<Mat>& L2M_f_QR) {
			...
			if (!tree->at(j)[nn].Eliminated) {
				if(pp != k) {
					if (!tree->at(j)[pp].Eliminated) { //case 4
						...
						// std::cout << "is_well_separated(j,nn,pp): " << is_well_separated(j,nn,pp) << std::endl;
						if(is_well_separated(j,nn,pp)) {
							// std::cout << "1: " << tree->at(j)[nn].M2L[pp].norm() << "	2: " << tree->at(j)[nn].P2P[pp].norm()/tree->at(j)[nn].M2L[pp].norm() << std::endl;
							if (nn < pp) { // using symmetry
				...
				if(pp != k) {
					if (!tree->at(j)[pp].Eliminated) { //case 2/3
						...
						// std::cout << "is_well_separated(j,nn,pp): " << is_well_separated(j,nn,pp) << std::endl;
						if(is_well_separated(j,nn,pp)) {
							// std::cout << "1: " << tree->at(j)[nn].M2L[pp].norm() << "	2: " << tree->at(j)[nn].P2L[pp].norm()/tree->at(j)[nn].M2L[pp].norm() << std::endl;
	...
	double findMemoryAfterFactorize() {
		...
		// commented the below as this is taken care by the neighbors at all levels
		// IL
		// for(int j=nLevels; j>=startLevel; --j) {
		// 	for (int k=0; k<nBoxesPerLevel[j]; ++k) {
		// 		for (size_t il = 0; il < tree->at(j)[k].interactionList.size(); il++) {
	...
	void eliminate_phase_efficient() {
		...
		for(int j=nLevels; j>=startLevel; --j) {
			...
			// std::cout << "tree->at(3)[385].NumMultipoles: " << tree->at(3)[385].NumMultipoles << std::endl;
			// std::cout << "tree->at(3)[345].M2L[385]: " << tree->at(3)[345].M2L[385].rows() << "," << tree->at(3)[345].M2L[385].cols() << std::endl;
			double timeInCompressFillinAndElimination = 0.0;
			start  = omp_get_wtime();
			double P2PFillInTime = 0;
			double P2LFillInTime = 0;
			...
			for (int k=0; k<nBoxesPerLevel[j]; ++k) {
				...
				double start_P2P = omp_get_wtime();
					...
					if (k == nn || k == pp) {
							...
							if (tree->at(j)[nn].P2P[pp].size() != 0) {
								...
								// if (j==3 && nn==231 && pp==239) {
									// std::cout << "compress_P2P j: " << j << " k: " << k << "	nn: " << nn << "	pp: " << pp << std::endl;
								// }
								// if (j==nLevels) {
				...
				double end_P2P = omp_get_wtime();
				// if (j==2) {
					// std::cout << "P2P compression DONE" << std::endl;
				// }
				double start_P2L = omp_get_wtime();
				...
				double end_P2L = omp_get_wtime();
				P2PFillInTime += end_P2P - start_P2P;
				P2LFillInTime += end_P2L - start_P2L;
				...
				// P2P.clear();
				// P2L_M2P.clear();
				double start_eliminate = omp_get_wtime();
				tree->at(j)[k].Eliminated = true;
				eliminate_cluster(j,k);
				double end_eliminate = omp_get_wtime();
				eliminateTime += end_eliminate - start_eliminate;
				// if (j==2) {
	   			// std::cout << "elimination DONE" << std::endl;
				// }
			}
			end = omp_get_wtime();
			timeInCompressFillinAndElimination = end - start;
			std::cout << "j: " << j << std::endl;
			std::cout << "initialTime: " << initialTime << std::endl;
			std::cout << "P2PFillInTime: " << P2PFillInTime << std::endl;
			std::cout << "P2LFillInTime: " << P2LFillInTime << std::endl;
			std::cout << "eliminateTime: " << eliminateTime << std::endl;
			std::cout << "timeInCompressFillinAndElimination: " << timeInCompressFillinAndElimination << std::endl;
		}
  ...
  void rhs_eliminate_phase_efficient() {
    for(int j=nLevels; j>=startLevel; --j) {
      ...
      for (int k=0; k<nBoxesPerLevel[j]; ++k) {
        ...
        rhs_eliminate_cluster(j,k);
				// std::cout << "j: " << j << "	k: done " << k << std::endl;
			}
		}
	}
    ...
    solve_particles_at_base_level(j);//now multipoles at level 1 are available
		// std::cout << "solve_particles_at_base_level done" << std::endl;
  ...
  void getx(Vec &x) {
	...
	void rankGrowthBetweenTwoBoxes() {
		int j = 2;
		int k = 0;
		int il = 4;
		Mat mat = K->getMatrix(tree->at(j)[k].chargeLocations, tree->at(j)[il].chargeLocations);
		int N1 = tree->at(j)[k].chargeLocations.size();
		int N2 = tree->at(j)[il].chargeLocations.size();
		// Eigen::JacobiSVD<Eigen::MatrixXcd, Eigen::ComputeThinU | Eigen::ComputeThinV> svd(mat);
		Eigen::JacobiSVD<Eigen::MatrixXcd> svd;
		svd.compute(mat, ComputeThinU | ComputeThinV);
		Eigen::MatrixXcd U = svd.matrixU();
		...
		std::cout << "N1, N2: " << N1 << "," << N2 << "	SVD Rank: " << svdRank << std::endl;
		// std::cout << "ACA Rank: " << acaRank << std::endl;
		Eigen::VectorXd svthin = svd.singularValues().segment(0,svdRank);
		Eigen::MatrixXcd Uthin = U.block(0,0,N1,svdRank);
		Eigen::MatrixXcd Vthin = V.block(0,0,N2,svdRank);
		...
		int ACARank;
		std::vector<int> row_indices, col_indices;
		row_indices = tree->at(j)[k].chargeLocations;
		col_indices = tree->at(j)[il].chargeLocations;//object of base class G_LowRank
		std::vector<int> row_bases, col_bases;
		Mat Ar;
		LowRank<kerneltype>* LR		=	new LowRank<kerneltype>(K, pow(10,-TOL_POW), row_indices, col_indices);
		LR->ACA_only_nodes2(row_bases, col_bases, ACARank, tree->at(j)[k].Ac, Ar);

		if(ACARank > 0) {
			for (int r = 0; r < row_bases.size(); r++) {
				tree->at(j)[k].incoming_checkPoints.push_back(row_indices[row_bases[r]]);
			}
			for (int c = 0; c < col_bases.size(); c++) {
				tree->at(j)[k].incoming_chargePoints.push_back(col_indices[col_bases[c]]);
			}
			...
			Mat Err = mat - tree->at(j)[k].Ac*D_lu.solve(Ar);
			double err = Err.norm()/mat.norm();
			std::cout << "ACA rank: " << ACARank << "	ErrACA: " << err << std::endl;
		}
		delete LR;
	}

	void rankGrowthBetweenBoxAndIL() {
		int j = 2;
		int k = 0;
		// int il = 4;
		std::vector<int> boxA_Nodes = tree->at(j)[k].chargeLocations;
		// getParticlesFromChildren_incoming_row(j, k, boxA_Nodes);
		std::vector<int> IL_Nodes;//indices
		for(int in=0; in<tree->at(j)[k].interactionList.size(); ++in) {
				int kIL = tree->at(j)[k].interactionList[in];
				std::vector<int> chargeLocations;
				// getParticlesFromChildren_incoming_col(j, kIL, chargeLocations);
				IL_Nodes.insert(IL_Nodes.end(), tree->at(j)[kIL].chargeLocations.begin(), tree->at(j)[kIL].chargeLocations.end());
		}
		Mat mat = K->getMatrix(boxA_Nodes, IL_Nodes);
		int N1 = boxA_Nodes.size();
		int N2 = IL_Nodes.size();
		// Eigen::JacobiSVD<Eigen::MatrixXcd, Eigen::ComputeThinU | Eigen::ComputeThinV> svd(mat);
		Eigen::JacobiSVD<Eigen::MatrixXcd> svd;
		svd.compute(mat, ComputeThinU | ComputeThinV);
		Eigen::MatrixXcd U = svd.matrixU();
		...
		col_indices = IL_Nodes;//object of base class G_LowRank
		std::vector<int> row_bases, col_bases;
		Mat Ar;
		LowRank<kerneltype>* LR		=	new LowRank<kerneltype>(K, pow(10,-TOL_POW), row_indices, col_indices);
		LR->ACA_only_nodes2(row_bases, col_bases, ACARank, tree->at(j)[k].Ac, Ar);
	...
	void viewRankStructure(std::string filename) {
		int *avgSVDRank = new int[nLevels];
		int *avgACARank = new int[nLevels];
		// std::cout << "1" << std::endl;
		std::ofstream myfile;
		myfile.open(filename.c_str());
		...
		myfile << "\\begin{tikzpicture}[yscale=-1]" << std::endl << std::endl;
		int cyanIntensity = 100;// reduce it by 20 for each increasing level from level 2
		double scalingFactor = 50.0/N;
		double fontSize = 20;
		for (size_t j = 0; j <= nLevels; j++) {
			int indexInMatrix = 0;
	  	for (size_t k = 0; k < nBoxesPerLevel[j]; k++) {
				tree->at(j)[k].indexInMatrix = indexInMatrix;
				indexInMatrix += tree->at(j)[k].chargeLocations.size();
			}
		}
		// std::cout << "2" << std::endl;
		// prints size of each row block
		for (size_t j = 2; j <= nLevels; j++) {
	  	for (size_t k = 0; k < nBoxesPerLevel[j]; k++) {
				double boxCenter_rows = (tree->at(j)[k].indexInMatrix+double(tree->at(j)[k].chargeLocations.size())/2)*scalingFactor;
				double boxCenter_cols = tree->at(j)[k].chargeLocations.size()/2*scalingFactor;
				myfile << "\\node at (" << -boxCenter_cols << "," << boxCenter_rows << ") {\\fontsize{" << fontSize << "}{8}\\selectfont " << tree->at(j)[k].chargeLocations.size() << "};" << std::endl << std::endl;
			}
			fontSize /= 4;
		}
		fontSize = 20;
		// prints size of each column block
		// std::cout << "3" << std::endl;
		for (size_t j = 2; j <= nLevels; j++) {
	  	for (size_t k = 0; k < nBoxesPerLevel[j]; k++) {
				double boxCenter_rows = (tree->at(j)[k].indexInMatrix+double(tree->at(j)[k].chargeLocations.size())/2)*scalingFactor;
				double boxCenter_cols = tree->at(j)[k].chargeLocations.size()/2*scalingFactor;
				myfile << "\\node at (" << boxCenter_rows << "," << -boxCenter_cols << ") {\\fontsize{" << fontSize << "}{8}\\selectfont " << tree->at(j)[k].chargeLocations.size() << "};" << std::endl << std::endl;
			}
			fontSize /= 4;
		}
		fontSize = 20;
		// std::cout << "4" << std::endl;
		for (size_t j = 2; j <= nLevels; j++) {
			int nBoxes = 0; // no. of boxes with non-zero interactions in level j
			avgSVDRank[j] = 0;
			avgACARank[j] = 0;
	  	for (size_t k = 0; k < nBoxesPerLevel[j]; k++) {
				for (size_t il_index = 0; il_index < tree->at(j)[k].interactionList.size(); il_index++) {
					// std::cout << "j: " << j << " k: " << k << "	il_index: " << il_index << " begin" << std::endl;
					int il = tree->at(j)[k].interactionList[il_index];
					Mat mat = K->getMatrix(tree->at(j)[k].chargeLocations, tree->at(j)[il].chargeLocations);
					// Eigen::JacobiSVD<Eigen::MatrixXcd, Eigen::ComputeThinU | Eigen::ComputeThinV> svd(mat);
					// std::cout << "mat.size: " << mat.rows() << "," << mat.cols() << std::endl;
					// std::cout << "4aa" << std::endl;
					int svdRank;
					Eigen::MatrixXcd mat_SVD;
					if (mat.size() > 0) {
						Eigen::JacobiSVD<Eigen::MatrixXcd> svd;
						svd.compute(mat, ComputeThinU | ComputeThinV);
						Eigen::VectorXd sv = svd.singularValues();
						...
						Eigen::MatrixXcd singularValueMatruxTruncated = Eigen::MatrixXcd::Zero(svdRank, svdRank);
						for (size_t i = 0; i < svdRank; i++) {
							singularValueMatruxTruncated(i,i) = svd.singularValues()(i);
						}
						mat_SVD = svd.matrixU().block(0,0,mat.rows(),svdRank) * singularValueMatruxTruncated * svd.matrixV().block(0,0,mat.cols(),svdRank).adjoint();
						// Eigen::MatrixXcd singularValueMatrixTruncated = Eigen::MatrixXcd::Zero(svd.singularValues().size(), svd.singularValues().size());
						// for (size_t i = 0; i < svd.singularValues().size(); i++) {
						// 	singularValueMatrixTruncated(i,i) = svd.singularValues()(i);
						// }
						// mat_SVD = svd.matrixU() * singularValueMatrixTruncated * svd.matrixV().adjoint();

					}
					else {
						svdRank = 0;
					}
					Eigen::MatrixXcd mat_ACA = tree->at(j)[k].L2P*tree->at(j)[k].M2L[il]*tree->at(j)[il].P2M;
					double err_ACA_mat = (mat - mat_ACA).norm()/mat.norm();
					double err_SVD_mat = (mat - mat_SVD).norm()/mat.norm();
					// std::cout << "j: " << j << " k: " << k << " il: " << il << "	" << std::round(-std::log(err_SVD_mat)/std::log(10)) << "," << std::round(-std::log(err_ACA_mat)/std::log(10)) << std::endl;
					// std::cout << "j: " << j << " k: " << k << " il: " << il << "	err_SVD_mat: " << err_SVD_mat << "	err_ACA_mat: " << err_ACA_mat << std::endl;
					// int acaRank = std::min(tree->at(j)[k].incoming_checkPoints.size(), tree->at(j)[il].outgoing_chargePoints.size());
					// std::cout << "4a" << std::endl;
					// int acaRank = tree->at(j)[k].L2P.cols();
					// int acaRank = tree->at(j)[il].P2M.rows();
					int acaRank = std::min(tree->at(j)[k].L2P.cols(), tree->at(j)[il].P2M.rows());
					if (tree->at(j)[k].chargeLocations.size() > 0 && tree->at(j)[il].chargeLocations.size() > 0) {
						avgSVDRank[j] += svdRank;
						avgACARank[j] += acaRank;
						nBoxes++;
					}
					// std::cout << "4b" << std::endl;
					myfile << "\\draw[ultra thin, fill=cyan!" << cyanIntensity << "] (" << tree->at(j)[il].indexInMatrix*scalingFactor << ",";
					myfile << tree->at(j)[k].indexInMatrix*scalingFactor << ") rectangle +(";
					myfile << tree->at(j)[il].chargeLocations.size()*scalingFactor << ",";
					myfile << tree->at(j)[k].chargeLocations.size()*scalingFactor << ");" << std::endl;
					double boxCenter_rows = (tree->at(j)[k].indexInMatrix+double(tree->at(j)[k].chargeLocations.size())/2)*scalingFactor;
					double boxCenter_cols = (tree->at(j)[il].indexInMatrix+double(tree->at(j)[il].chargeLocations.size())/2)*scalingFactor;
					myfile << "\\node at (" << boxCenter_cols << "," << boxCenter_rows << ") {\\fontsize{" << fontSize << "}{8}\\selectfont " << svdRank << "," << acaRank << "};" << std::endl << std::endl;
					myfile << "\\node at (" << boxCenter_cols << "," << boxCenter_rows+15*scalingFactor << ") {\\fontsize{" << fontSize << "}{8}\\selectfont " << std::round(-std::log(err_SVD_mat)/std::log(10)) << "," << std::round(-std::log(err_ACA_mat)/std::log(10)) << "};" << std::endl << std::endl;
					// tree->at(j)[k].indexInMatrix, tree->at(j)[il].indexInMatrix
					// tree->at(j)[k].chargeLocations, tree->at(j)[il].chargeLocations
					// std::cout << "j: " << j << " k: " << k << "	il_index: " << il_index << " end" << std::endl;
				}
	  	}
			fontSize /= 4;
			cyanIntensity -= 30;
		...
		int j = nLevels;
		for (size_t k = 0; k < nBoxesPerLevel[j]; k++) {
			for (size_t n_index = 0; n_index < tree->at(j)[k].neighborList.size(); n_index++) {
				int n = tree->at(j)[k].neighborList[n_index];
				myfile << "\\draw[ultra thin, fill=red] (" << tree->at(j)[n].indexInMatrix*scalingFactor << ",";
				myfile << tree->at(j)[k].indexInMatrix*scalingFactor << ") rectangle +(";
				myfile << tree->at(j)[n].chargeLocations.size()*scalingFactor << ",";
				myfile << tree->at(j)[k].chargeLocations.size()*scalingFactor << ");" << std::endl;
				// double boxCenter_rows = (tree->at(j)[k].indexInMatrix+double(tree->at(j)[k].chargeLocations.size())/2)*scalingFactor;
				// double boxCenter_cols = (tree->at(j)[n].indexInMatrix+double(tree->at(j)[n].chargeLocations.size())/2)*scalingFactor;
				// myfile << "\\node at (" << boxCenter_cols << "," << boxCenter_rows << ") {\\tiny{$" << tree->at(j)[k].chargeLocations.size() << "\\times" << tree->at(j)[n].chargeLocations.size() << "$}};" << std::endl << std::endl;
			}
		}
		// std::cout << "5" << std::endl;
		...
		std::cout << "Electrical size of boxes at different levels" << std::endl;
		for (size_t j = 2; j <= nLevels; j++) {
			std::cout << "j: " << boxSideLength[j] << std::endl;
		}

		std::cout << "Average SVD and ACA ranks" << std::endl;
		for (size_t j = 2; j <= nLevels; j++) {
			std::cout << "j: " << avgSVDRank[j] << "," << avgACARank[j] << std::endl;
		}
		delete avgSVDRank;
		delete avgACARank;
	}
